    ...
    const getAllNodes = function (node, lastHTMLElement = null, lastSelectOrDataListElement = null) {
      if (node.nodeType == 1 || node.nodeType == 11) {
        if (node.nodeType == 11) {
          ...
          if (htmlTagsInlineIgnore.indexOf(nodeName) !== -1 || isNoTranslateNode(node) || node.classList.contains("notranslate") || node.getAttribute("translate") === "no" || node.isContentEditable || node.classList.contains("material-icons") ||
    ...
    const onTabVisible = function () {
      chrome.runtime.sendMessage({
        ...
        if (result === "und") {
          ...
          if (twpConfig.get("alwaysTranslateSites").indexOf(tabHostName) !== -1 || location.hostname === "pdf.translatewebpages.org" && twpConfig.get("neverTranslateSites").indexOf(tabHostName) === -1) {
            pageTranslator.translatePage();
          }
        } else {
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/9.9.0.3/pageTranslator.js.map
